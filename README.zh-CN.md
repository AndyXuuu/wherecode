...
- `command_center/`：Next.js + Tailwind（Node.js 生态）
- `control_center/`：FastAPI + Pydantic（Python 生态）
- `action_layer/`：Agent 运行层（可按模型/运行时扩展）

为避免跨栈耦合，依赖与环境文件应放在各自子项目内。  
根目录不放置 `requirements.txt`、`.env.example`、`run.sh` 这类全局依赖文件。
